...
export function TestimonialCarousel({ items }: { items: Testimonial[] }) {
  ...
  return (
    <div className="carousel-shell">
      ...
      >
        {items.map((item) => (
          <article className={`quote-card ${item.tone}`} key={item.source}>
            <div className="quote-mark">“</div>
            <p>{item.quote}</p>
            <div className="quote-source"><span className="source-dot"></span><strong>{item.source}</strong></div>
          </article>
        ))}
      </div>
      <div className="carousel-controls">
        <span>Drag or swipe to read more</span>
        <div>
